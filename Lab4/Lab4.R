...
legend(-1, 1.5, first_m$Год, cex = 0.7, fill=rainbow(length(first_m$Год)))
...
legend(max(events_gold$Год) - 2, 53, c("США", "Китай", "Япония", "Великобритания", "Россия", "Австралия", "Нидерланды"), fill=c("#3be8b0", "#1aafd0", "#6a67ce", "#ffb900", "orchid", "#2e3c54", "brown"))
...
legend(max(events_prizes$Год) - 1, 137, c("США", "Китай", "Япония", "Великобритания", "Россия", "Австралия", "Нидерланды"), fill=c("#3be8b0", "#1aafd0", "#6a67ce", "#ffb900", "orchid", "#2e3c54", "brown"))
...
legend(min(prize_6_f$Год), 7.2, cex=0.7 ,c("Мужчины", "Женщины"), fill=c("navyblue", "hotpink","darkolivegreen3"))
axis(side=1, at=prize_m$Год)

prize_grouped = data.frame(Призовых_М=prize_6_m$Призовых, Призовых_Ж=prize_6_f$Призовых)
barplot(height=t(as.matrix(prize_grouped)), beside=TRUE, xlab="Год", ylab="Количество", names.arg=prize_6_f$Год, col=c("navyblue", "hotpink","darkolivegreen3"), main="Количество призовых мест России\nпо вольной борьбе за последние 6 ОИ")

prize_6_sum <- sapply(prize_grouped, sum)
pie(prize_6_sum, labels=c(prize_6_sum["Призовых_М"], prize_6_sum["Призовых_Ж"]), col=c("navyblue", "hotpink","darkolivegreen3"), main="Всего призовых мест у М и Ж из России\nпо вольной борьбе за последние 6 ОИ")
